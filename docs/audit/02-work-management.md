...
## Follow-up: a recurring task spawns one successor, 2026-09-05

The duplication half of WORK-006 is closed. Job-based generation is not, and is
recorded below as still open.

`updateTaskStatus` never read the task's prior status, so setting an
already-completed task to "completed" again inserted another occurrence. The
insert carried no key of any kind, so a double-click or a client retry did the
same. The duplicates were indistinguishable from real work — same title, same
assignee, same due date, nothing marking which one was the accident — so this
corrupted a task list quietly rather than failing visibly.

The command now checks the prior status, but a check cannot win a race: two
requests can both read "not completed" before either writes. So the guarantee
is the shape of the data. `task.recurrence_parent_id` points at the occurrence
a task was spawned from and carries a partial unique index, making a second
successor unrecordable rather than merely detected. The unique violation is
swallowed on purpose — the successor exists either way, which is what the
caller wanted, and reporting a failure for a task that completed successfully
would be a worse lie than silence. Any other insert error is reported.

The pointer earns its place beyond this fix. Occurrences were unlinked copies
with nothing tying a series together, which is the larger gap in P1-TSK-05. A
chain of predecessors is not a series entity and this does not create one, but
it is the first thing that makes a series recoverable from the data at all.

Five assertions: the duplicate is refused, exactly one successor survives, many
tasks can hold a null predecessor at once (the index has to be partial or the
second ordinary task ever created would collide with the first), and a task
cannot be its own successor.

**Still open on WORK-006:** generation is a side effect of a user action rather
