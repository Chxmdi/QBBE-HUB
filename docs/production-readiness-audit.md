...
| Gmail OAuth and inbox sync | 🟡 Partial — code complete, live gate pending | OAuth uses `gmail.readonly` + `gmail.send`, persists granted scopes, preserves refresh tokens on reconnect, supports compose/reply and on-demand full-message retrieval, and reconciles paginated Gmail history. Authenticated Pub/Sub validates Google issuer/audience/service account/expiry, queues durable reconciliation immediately, and has a one-minute recovery worker plus the existing periodic sweep. Full rebuild removes stale metadata only after a successful provider read. Real QBBE Google/Pub/Sub staging validation and authorized test recipients remain required before INT-GMAIL can be verified. |
| Google Calendar integration | 🟡 Partial — code complete, live gate pending | OAuth uses `calendar.events`; overlay ingest has paginated sync tokens, cancellation reconciliation and full rebuild after expiry. Hub-owned meeting/event writes now use deterministic Google event IDs so POST retries converge rather than duplicate; 409 retries PATCH the existing event and update-404 recreates a missing remote event. User-provided meeting links remain untouched, local Hub writes survive provider failure, and existing DST/timezone round-trip tests cover organization wall time. Real QBBE Calendar staging validation and re-consent remain required for INT-CALENDAR. |
| Google Drive integration | 🟡 Partial — code complete, live gate pending | Per-user OAuth uses read-only Drive metadata. Imported metadata is owner-scoped (`private`) instead of organization-wide, so a connection cannot widen QBBE access. Initial/incremental sync uses persisted change cursors, reconciles moved/removed/inaccessible items, and full rebuild upserts before stale pruning so provider/DB failure cannot empty the last known mirror. File bytes and Google permissions are never copied/changed. Real QBBE Drive staging validation remains required for INT-DRIVE. |
| Volunteer Management System integration | 🟡 Partial — code complete, live contract pending | Admin can explicitly link active Hub members to VMS identities; the worker normalizes identity/availability plus minimal assignment references, reconciles authoritative assignment snapshots, preserves identity-only responses, and never creates/deletes Hub tasks. Connect validates the provider envelope and failures surface actionable health. The actual provider contract and QBBE credentials remain required for live INT-VMS acceptance. |
