...
test("a project records a risk trigger, an issue plan, a decision, and a request", async ({
  ...
  await risk.getByLabel("What might happen", { exact: true }).fill("The hall cancels");
  await risk.getByLabel("What would make it happen", { exact: true }).fill("Notice inside two weeks");
  ...
  await issue.getByLabel("What has happened", { exact: true }).fill("The printer failed");
  await issue.getByLabel("Impact", { exact: true }).fill("Packets cannot be printed");
  await issue.getByLabel("Resolution plan", { exact: true }).fill("Borrow the school copier");
  ...
  await decision.getByLabel("The decision", { exact: true }).fill("Hold the event indoors");
  await decision.getByLabel("Rationale", { exact: true }).fill("Rain is forecast");
  await decision.getByLabel("Alternatives considered", { exact: true }).fill("Postpone");
  await decision.getByLabel("Affected records", { exact: true }).fill("Saturday session");
  await decision.getByLabel("What would reopen it", { exact: true }).fill("A dry forecast by Thursday");
  ...
  await request.getByLabel("Ask", { exact: true }).selectOption({ label: "QA Staff" });
  await request.getByLabel("Due", { exact: true }).fill("2026-10-01");
  await request.getByLabel("What needs deciding", { exact: true }).fill("Which room do we use?");
  await request.getByRole("button", { name: "Send request" }).click();
  await expect(request).not.toBeVisible({ timeout: 30_000 });

  await page.reload();
  await expect(page.getByText("Notice inside two weeks").filter({ visible: true })).toBeVisible();
  await expect(page.getByText("Packets cannot be printed").filter({ visible: true })).toBeVisible();
  await expect(page.getByText("Borrow the school copier").filter({ visible: true })).toBeVisible();
  await expect(page.getByText("Hold the event indoors").filter({ visible: true })).toBeVisible();
  await expect(page.getByText("Rain is forecast").filter({ visible: true })).toBeVisible();
  await expect(page.getByText("A dry forecast by Thursday").filter({ visible: true })).toBeVisible();
  await expect(page.getByText("Which room do we use?").filter({ visible: true })).toBeVisible();
});
