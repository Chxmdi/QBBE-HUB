...
test("a failed search says so instead of claiming there are no results", async ({ page }) => {
  ...
  // Scoped to the palette: Next.js keeps its own empty role="alert" route
  // announcer on every page.
  const palette = page.getByRole("dialog", { name: "Command palette" });
  await palette.getByRole("combobox", { name: "Search" }).fill("workshop");
  await expect(palette.getByRole("alert")).toContainText("Search isn't available right now.");
  ...
  await palette.getByRole("button", { name: "Try again" }).click();
